...
getwd()
#### include dxiaiR libraries to .libPaths()
if (!file.exists("~/.Renviron")) {file.create("~/.Renviron")}
# append the dxiaiR library path to to R
renviron = stringi::stri_read_lines("~/.Renviron")
if(!stringi::stri_detect_fixed(renviron,"~/.R", "library/dxiaiR")){
    cat(paste0("R_LIBS_USER=","~/.R", "library/dxiaiR"), file = "~/.Renviron", append = TRUE)
    # Restart R Session
    .rs.restartR()
    .libPaths()
}
#### 
# paste0("R_LIBS_USER=",paste0(getwd(),"/dxiaiR"))
# .libPaths()
# getwd()
# # Set the directory to the application path
# setwd(dirname(rstudioapi::getSourceEditorContext()$path))
...
if (!require('dxiaiR')) install.packages('dxiaiR'); library(dxiaiR)
...
# Create a preprocessed dataframe. Input 'ZHAW_Evento_all-docs-content.Rda'
df <- evto.create.data.frame(dd,nrow(dd))


###########################################################################
# Store the digital collection as dataframe into the hidden temp directory
saveRDS(df, file = paste0(tmpDir,"/ZHAW_Evento_all_preprocessed.Rda"))

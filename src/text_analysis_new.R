...
# if (!require('zhawdatalibr')) install.packages('zhawdatalibr'); library(zhawdatalibr)
...
course_text <- function(x){
    x %>% 
        ...
        stri_replace_all(" ///Version: ", regex = "Version") %>%
        stri_replace_all(" ///Datum: Diese Information", regex = "Diese Information") %>%
...
for (i in 1:10){
# for (i in 1:nrow(dd)){
    mydf[i,1] <- Evto.Course.ID(dd$course[i])
    mydf[i,2] <- Evto.Course.title(dd$course[i])
    x <- dd$text[i]
